...
router.get("/", async (req, res) => {
	try {
		...
		if (req.session.visitedProducts) {
			req.session.visitedProducts.push(prodID);
		} else {
			req.session.visitedProducts = [prodID];
		}
		...
		const productData = await Product.aggregate([
			...
			},
			{
				$set: {
					...
					"reviews.ecommerce": {
						$arrayElemAt: ["$reviews.ecommerce", 0],
					},
				},
			},
			{
...
router.post("/search", async (req, res) => {
	try {
		...
		if (req.session.searchQueries) {
			req.session.searchQueries.push(query);
		} else {
			req.session.searchQueries = [query];
		}
				...
				if (filter.hasOwnProperty(key) && filter[key].value.length) {
					if (filter[key].type === "range") {
						filter_query[filter[key].identifier] = {
							$gte: filter[key].value[0],
							$lte: filter[key].value[1],
						};
					} else {
						let temp = [];
